  ...
  async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const decodedToken: AuthToken['data'] & AuthToken['meta'] = jwt_decode(data.authToken);
        if (decodedToken.rememberMe) cookiesManager.setCookie('authToken', data.authToken, true);
        else cookiesManager.setCookie('authToken', data.authToken, false);
      } else if (response.status === 401) cookiesManager.deleteCookie('authToken');
      return data;
    } catch (error) {
